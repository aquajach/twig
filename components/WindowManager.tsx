...
const spring = { type: 'spring' as const, duration: 0.35, bounce: 0.1 };
      ...
      <AnimatePresence mode="popLayout" custom={toDesktop}>
        {activeApp ? (
          <motion.div
            ...
            animate={{ scale: 1 }}
            exit="exit"
            variants={{ exit: (minimize: boolean) => (minimize ? { scale: 0 } : { opacity: 0 }) }}
            transition={spring}
            style={{ transformOrigin: 'bottom center' }}
            className="absolute inset-0 rounded-t-[var(--radius-container)] overflow-hidden shadow-lg/40"
          >
